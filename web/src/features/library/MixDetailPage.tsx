...
import { toAnalysisSummary } from "../../api/mixAdapters";
...
export function MixDetailPage() {
  ...
  return (
    <section className="route-page mix-detail-page" aria-labelledby="page-heading">
      ...
      <p className="eyebrow">Completed master</p>
      ...
      <ABPlayer artifacts={mix.artifacts} sourceFilename={mix.sourceFilename} suggestedDownloadName={mix.suggestedDownloadName} />
      ...
      <section className="rig-launch" aria-labelledby="rig-launch-heading"><div><h2 id="rig-launch-heading">Listening rig</h2><p>Open an optional, non-strobing visual reference. Your playback is unchanged.</p></div><Button onClick={() => setRigOpen(true)} ref={rigButtonRef} tone="secondary">Open listening rig</Button></section>
      {rigOpen ? <Suspense fallback={<p className="rig-loading" role="status">Opening listening rig…</p>}><LazyRigPanel mixId={mix.id} onClose={closeRig} /></Suspense> : null}
    </section>
  );
}
