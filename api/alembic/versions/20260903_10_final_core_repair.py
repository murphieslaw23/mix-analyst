...
down_revision: Union[str, Sequence[str], None] = "20260902_09_batches"
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("upload_sessions") as batch_op:
        batch_op.add_column(sa.Column("promotion_state", sa.String(length=32), nullable=False, server_default="NONE"))
        batch_op.add_column(sa.Column("final_key", sa.String(length=512), nullable=True))
        ...
        batch_op.create_foreign_key("fk_upload_sessions_media_asset_id_media_assets", "media_assets", ["media_asset_id"], ["id"])
        batch_op.create_foreign_key("fk_upload_sessions_mix_id_mixes", "mixes", ["mix_id"], ["id"])
...
def downgrade() -> None:
    with op.batch_alter_table("upload_sessions") as batch_op:
        ...
        batch_op.drop_constraint("fk_upload_sessions_media_asset_id_media_assets", type_="foreignkey")
