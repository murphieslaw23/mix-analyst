"""Privacy-safe in-memory operational metrics for the API.

Only allowlisted tag keys may be recorded. Media filenames, user content,
storage keys, and tokens must never become metric labels.
"""
...
def record_counter(
    ...
    """Increment an in-memory counter for an allowlisted tag set."""
    _validate_name(name)
...
def record_gauge(
    ...
    """Set an in-memory gauge for an allowlisted tag set."""
    _validate_name(name)
...
def snapshot() -> dict[str, dict[str, float]]:
    """Return a point-in-time copy of counters and gauges."""
    with _lock:
        return {"counters": dict(_counters), "gauges": dict(_gauges)}


def reset() -> None:
    """Clear all recorded series. Intended for tests."""
    with _lock:
        _counters.clear()
        _gauges.clear()
