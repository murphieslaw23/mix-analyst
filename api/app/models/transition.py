...
class TransitionEvent(Base):
    ...
    cue_in_time: Mapped[float] = mapped_column(Float, nullable=False)
    cue_out_time: Mapped[float] = mapped_column(Float, nullable=False)

    transition_type: str = Column(String(50), default="SMOOTH_BLEND", nullable=False)
    energy_delta: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    tempo_shift_bpm: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    ...
    mix: Mapped[Mix] = relationship("Mix", back_populates="transitions")
