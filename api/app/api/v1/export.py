...
def export_cue_sheet(
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="{mix.id}.cue"'},
    )
...
def export_rekordbox(
    ...
    transitions = [
        ...
        for event in db.query(TransitionEvent)
        .filter(TransitionEvent.mix_id == mix_id)
        .order_by(TransitionEvent.transition_index)
        .all()
    ]
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{mix.id}_rekordbox.xml"'
        },
    )
...
def export_traktor(
    ...
    return Response(
        content=nml_content,
        media_type="application/xml",
        headers={"Content-Disposition": f'attachment; filename="{mix.id}_traktor.nml"'},
    )
