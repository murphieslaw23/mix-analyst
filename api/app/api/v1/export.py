...
def export_rekordbox(mix_id: str, db: Session = Depends(get_db)):
    ...
    transitions_db = (
        db.query(Transition)
        .filter(Transition.media_id == mix_id)
        .order_by(Transition.start_time)
        .all()
    )
    transitions = []
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{media.id}_rekordbox.xml"'
        },
    )
...
def export_traktor(mix_id: str, db: Session = Depends(get_db)):
    ...
    return Response(
        ...
        media_type="application/xml",
        headers={
            "Content-Disposition": f'attachment; filename="{media.id}_traktor.nml"'
        },
    )
