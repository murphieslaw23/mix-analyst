...
def _cursor_error() -> HTTPException:
    return HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="The jobs cursor is invalid or has expired. Refresh the jobs list and try again.")
...
def _encode_cursor(payload: dict[str, object]) -> str:
    ...
    signature = hmac.new(settings.auth_jwt_secret.encode(), encoded_payload, hashlib.sha256).digest()
...
def _decode_cursor(value: str, project_id: str) -> dict[str, object]:
    try:
        ...
        expected = hmac.new(settings.auth_jwt_secret.encode(), payload_bytes, hashlib.sha256).digest()
        if not hmac.compare_digest(signature, expected):
            raise ValueError
        payload = json.loads(payload_bytes)
        if not isinstance(payload, dict) or payload.get("v") != 1 or payload.get("project_id") != project_id:
            raise ValueError
        for key in ("snapshot_created_at", "snapshot_id", "after_created_at", "after_id", "total"):
            if key not in payload:
                raise ValueError
        if not all(isinstance(payload[key], str) for key in ("snapshot_created_at", "snapshot_id", "after_created_at", "after_id")):
            raise ValueError
...
def _before_or_equal(created_at: datetime, job_id: str):
    return or_(Job.created_at < created_at, and_(Job.created_at == created_at, Job.id <= job_id))
...
def list_jobs(
    ...
    if cursor is not None and page is not None:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="Use either page or cursor, not both.")
    ...
    if cursor_payload is None:
        ...
        snapshot_created_at = datetime.fromisoformat(str(cursor_payload["snapshot_created_at"]))
        after_created_at = datetime.fromisoformat(str(cursor_payload["after_created_at"]))
    ...
    if cursor_payload is None and candidates:
        ...
        total = db.scalar(
            select(func.count()).select_from(
                scoped.where(_before_or_equal(snapshot_head.created_at, snapshot_head.id)).subquery()
            )
        ) or 0
    ...
    next_cursor = _encode_cursor({
        ...
        "snapshot_created_at": (snapshot.created_at if snapshot else datetime.fromisoformat(str(cursor_payload["snapshot_created_at"]))).isoformat(),
        "snapshot_id": snapshot.id if snapshot else str(cursor_payload["snapshot_id"]),
