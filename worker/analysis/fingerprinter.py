...
def generate_audio_fingerprint(audio_path: Path, offset_seconds: float, duration_seconds: float = 120.0) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        res = subprocess.run(cmd, capture_output=True, text=True, check=True, timeout=25)
        ...
        try:
            ...
            y, sr = read_audio_window(audio_path, offset_seconds, min(duration_seconds, 30.0), target_sr=11025)
            ...
            fp_repr = ",".join(str(int("".join(map(str, row[:32])), 2)) for row in quantized)
            return {
...
def query_acoustid_metadata(fingerprint: str, duration: float) -> Optional[Dict[str, Any]]:
    """
            ...
            if response.status == 200:
                ...
                if results and len(results) > 0:
                    ...
                    if recordings:
                        ...
                        artist_name = ", ".join(a.get("name", "Unknown Artist") for a in artists) if artists else "Unknown Artist"
...
def partition_mix_segments(duration_seconds: float, avg_track_length: float = 240.0) -> List[Dict[str, float]]:
    """
    ...
    if duration_seconds <= 180.0:
        return [{"segment_index": 1, "start_time_seconds": 0.0, "end_time_seconds": duration_seconds, "duration_seconds": duration_seconds}]
