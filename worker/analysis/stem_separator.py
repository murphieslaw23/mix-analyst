...
class StemSeparatorEngine:
    ...
    @staticmethod
    def analyze_bassline_and_collision(
        bass_audio: np.ndarray, drums_audio: np.ndarray, sample_rate: int
    ) -> dict[str, Any]:
        ...
        denom = (
            np.linalg.norm(bass_fft[sub_mask]) * np.linalg.norm(drums_fft[sub_mask])
        ) + 1e-6
        ...
        return {
            ...
            if collision_score < 0.4
            else "moderate_clash"
            if collision_score < 0.7
            else "high_clash",
