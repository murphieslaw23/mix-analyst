...
def test_job_list_is_paginated_and_never_discloses_another_project(
    client, user_a_token, job
):
...
def test_job_list_cursor_keeps_a_complete_snapshot_during_concurrent_inserts(
        ...
        db.add_all(
            [
                ...
                Job(
                    ...
                    status=JobStatus.QUEUED,
                    progress_percent=0.0,
                    created_at=datetime(2030, 1, 3, tzinfo=timezone.utc),
                ),
            ]
        )
        db.commit()
    ...
    first_page = test_client.get(
        "/api/v1/jobs?limit=2", headers={"Authorization": f"Bearer {user_a_token}"}
    )
    assert first_page.status_code == 200
    ...
    assert "cursor-arrived-later" not in {
...
def test_authenticated_master_command_is_queued_until_worker_output_exists(
    client, user_a_token
):
    ...
    try:
        outbox = (
            ...
            .filter(OutboxMessage.aggregate_id == body["id"])
            .one()
        )
...
def test_upgrade_preserves_unattributable_legacy_custom_preset_as_explicit_shared_profile(
    tmp_path,
):
    ...
    with engine.begin() as connection:
        connection.execute(
            text(
                "INSERT INTO mastering_presets (id, name, is_builtin) VALUES ('legacy-custom', 'Legacy Custom', 0)"
            )
        )
...
def test_mix_artifacts_are_presented_as_owner_scoped_metadata(client, user_a_token):
    ...
    try:
        media = MediaAsset(
            id="artifact-media-a",
            project_id="project-a",
            original_filename="Owned Mix.wav",
        ...
        db.add_all(
            [
                ...
                Artifact(
                    id="artifact-metadata-a",
                    project_id="project-a",
                    mix_id=mix.id,
                    role="metadata",
                    key="projects/project-a/artifacts/metadata/v1/" + "c" * 64,
                    sha256="d" * 64,
                    algorithm_version="v1",
                    media_type="application/json",
                    byte_length=4,
                    report={
                        "suggested_download_name": "Owned Mix [Tekno].wav",
                        "genre": "Tekno",
                    },
                ),
            ]
    ...
    assert (
        source["download_url"]
        == "/api/v1/mixes/artifact-mix-a/artifacts/artifact-source-a/download"
    )
    assert "/tmp/" not in source["download_url"]


def test_artifact_download_uses_role_specific_names_without_weakening_project_scope(
    client, user_a_token, user_b_token, monkeypatch, tmp_path
):
    ...
    monkeypatch.setattr(
        "api.app.api.v1.mixes.settings.storage_root", str(tmp_path / "storage")
    )
    test_client, session_factory = client
    ...
    try:
        media = MediaAsset(
            id="download-media-a",
            project_id="project-a",
            original_filename="Floor Tool 01.aiff",
        ...
        db.add_all(
            [
                ...
                Artifact(
                    ...
                    byte_length=2,
                    report={
                        "suggested_download_name": "Floor Tool 01 [SYCO23 Master].wav"
                    },
                ),
            ]
        )
    ...
    headers = {"Authorization": f"Bearer {user_a_token}"}
    source = test_client.get(
        "/api/v1/mixes/download-mix-a/artifacts/download-source-a/download",
        headers=headers,
    )
    mastered = test_client.get(
        "/api/v1/mixes/download-mix-a/artifacts/download-mastered-a/download",
        headers=headers,
    )
    ...
    assert (
        f"filename*=utf-8''{quote('Floor Tool 01.aiff')}"
        in source.headers["content-disposition"]
    )
    assert mastered.status_code == 200
    assert (
        f"filename*=utf-8''{quote('Floor Tool 01 [SYCO23 Master].wav')}"
        in mastered.headers["content-disposition"]
    )
...
def test_user_cannot_read_another_users_upload_session(
    client, user_a_token, user_b_upload
):
    test_client, _ = client
    response = test_client.get(
...
def test_unauthenticated_upload_initialization_is_rejected(
...
def test_user_cannot_append_to_another_users_upload_session(
    client, user_a_token, user_b_upload
):
    test_client, _ = client
...
def test_upload_completion_assigns_the_principal_project(
    client, user_b_token, monkeypatch, tmp_path
):
    monkeypatch.setattr(
        "api.app.api.v1.uploads.storage", StorageService(str(tmp_path / "storage"))
    )
    monkeypatch.setattr(
    ...
    try:
        media = (
            db.query(MediaAsset)
            .filter(MediaAsset.id == response.json()["media_asset_id"])
            .one()
        )
