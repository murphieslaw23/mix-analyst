...
def generate_synthetic_audio(duration_sec: float = 30.0, sample_rate: int = 22050, bpm: float = 120.0, freq_hz: float = 440.0) -> bytes:
    ...
    with wave.open(buffer, 'wb') as wav_file:
        ...
        for i in range(num_samples):
            ...
            if beat_pos < int(sample_rate * 0.05):
                ...
                kick = 0.8 * math.sin(2.0 * math.pi * 60.0 * (beat_pos / sample_rate)) * decay
            else:
                kick = 0.0
...
def generate_multi_track_mix(duration_sec: float = 60.0, sample_rate: int = 22050) -> bytes:
    ...
    with wave.open(buffer, 'wb') as wav_file:
        ...
        for i in range(num_samples):
            ...
            t1_kick = 0.7 * math.sin(2.0 * math.pi * 65.0 * (t1_pos / sample_rate)) * math.exp(-t1_pos / (sample_rate * 0.02)) if t1_pos < sample_rate * 0.05 else 0.0
            
            # Tone 2 (659.25 Hz)
            t2_beat_interval = int(sample_rate * (60.0 / 130.0))
            t2_tone = 0.4 * math.sin(2.0 * math.pi * 659.25 * t)
            t2_pos = i % t2_beat_interval
            t2_kick = 0.7 * math.sin(2.0 * math.pi * 70.0 * (t2_pos / sample_rate)) * math.exp(-t2_pos / (sample_rate * 0.02)) if t2_pos < sample_rate * 0.05 else 0.0
